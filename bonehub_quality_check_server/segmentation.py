"""Reading an uploaded segmentation and writing it into the dataset.
...
def _check_geometry(segmentation: sitk.ImageFileReader, image: sitk.ImageFileReader) -> None:
    ...
    if image.GetDimension() == 3 and not np.allclose(_affine(segmentation), _affine(image), atol=_GEOMETRY_TOLERANCE):
        raise SegmentationError(
