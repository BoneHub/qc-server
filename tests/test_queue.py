...
class IndexEligibilityTests(QCTestCase):
    ...
    def test_only_unchecked_segmentations_are_queued(self):
        self.builder.add_dataset(1)
        self.builder.add_subject(1, 1, segmentation={"FEMUR_LEFT": 2})  # generated, unchecked
        self.builder.add_subject(1, 2, segmentation={"FEMUR_LEFT": 3})  # already passed QC
        self.builder.add_subject(1, 3, segmentation={"FEMUR_LEFT": 1})  # from the original source
    ...
    def test_a_subject_with_any_unchecked_label_is_queued(self):
        self.builder.add_subject(1, 1, segmentation={"FEMUR_LEFT": 3, "FEMUR_RIGHT": 2})
    ...
    def test_a_subject_with_no_image_is_never_queued(self):
        self.builder.add_subject(1, 1, segmentation={"FEMUR_LEFT": 2}, image=False)
        self.assertEqual(self.keys(self.make_store()), [])
    ...
    def test_a_subject_whose_image_file_is_missing_is_skipped(self):
        ...
        self.builder.add_subject(1, 1, segmentation={"FEMUR_LEFT": 2}, write_image_file=False)
        self.builder.add_subject(1, 2, segmentation={"FEMUR_LEFT": 2})
        self.assertEqual(self.keys(self.make_store()), ["001_000002"])

    def test_the_eligible_values_are_configurable(self):
        self.builder.add_subject(1, 1, segmentation={"FEMUR_LEFT": 1})
        self.builder.add_subject(1, 2, segmentation={"FEMUR_LEFT": 2})
        store = self.make_store(eligible_label_values=[1])
        self.assertEqual(self.keys(store), ["001_000001"])

    def test_the_queue_is_ordered_by_dataset_then_subject(self):
        self.builder.add_subject(2, 5, segmentation={"FEMUR_LEFT": 2})
        self.builder.add_subject(1, 9, segmentation={"FEMUR_LEFT": 2})
        self.builder.add_subject(1, 2, segmentation={"FEMUR_LEFT": 2})
