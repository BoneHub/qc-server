...
temporary folder, which is what the server is pointed at::
    ...
    <root>/Dataset_001/Segmentation/001_000001.nii.gz
"""
...
#: Voxel grid used for every fixture image. Small enough that a suite run stays fast.
SHAPE = (6, 6, 6)
...
def write_nifti(path: Path, data: np.ndarray, affine: np.ndarray | None = None) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    image = nib.Nifti1Image(data.astype(np.int16), AFFINE if affine is None else affine)
    nib.save(image, str(path))
    return path


def segmentation_array(labels, shape=SHAPE) -> np.ndarray:
    """A volume holding one small cube per label, each painted with its voxel value."""
...
class DatasetBuilder:
    ...
    def add_dataset(self, dataset_id: int, name: str | None = None) -> "DatasetBuilder":
    ...
    def add_subject(
        ...
        **extra,
        ...
        ``write_image_file`` / ``write_segmentation_file`` default to what the metadata
        claims, so a test can create the mismatch of a subject whose file is missing.
        """
        ...
        if image if write_image_file is None else write_image_file:
            write_nifti(dataset_dir / "Image" / f"{key}.nii.gz", np.zeros(SHAPE, dtype=np.int16))

        present = sorted(label for label, value in (segmentation or {}).items() if value not in {0, -1})
        if bool(present) if write_segmentation_file is None else write_segmentation_file:
            write_nifti(dataset_dir / "Segmentation" / f"{key}.nii.gz", segmentation_array(present))
        return self
    ...
    def segmentation_file(self, dataset_id: int, subject_id: int) -> Path:
        padded = str(dataset_id).zfill(DATASET_ZFILL)
        return self.root / f"Dataset_{padded}" / "Segmentation" / f"{subject_key(dataset_id, subject_id)}.nii.gz"
    ...
    def labels_in_segmentation(self, dataset_id: int, subject_id: int) -> set:
        ...
        data = np.asanyarray(nib.load(str(self.segmentation_file(dataset_id, subject_id))).dataobj)
        value_to_name = {label.value: label.name for label in BoneLabelMap}
        return {value_to_name[int(v)] for v in np.unique(data) if int(v) != 0}
...
class QCTestCase(unittest.TestCase):
    ...
    def default_dataset(self, n_subjects: int = 3, dataset_id: int = 1) -> None:
        """One dataset whose subjects all carry unchecked (value 2) segmentations."""
        self.builder.add_dataset(dataset_id)
        for subject_id in range(1, n_subjects + 1):
            self.builder.add_subject(dataset_id, subject_id, segmentation={"FEMUR_LEFT": 2, "FEMUR_RIGHT": 2})

    def upload_file(self, labels, affine: np.ndarray | None = None, shape=SHAPE) -> Path:
        """A segmentation as a client would submit it, written outside the dataset folder."""
        path = self.tmp_path / "uploads" / f"upload_{next(self._upload_counter)}.nii.gz"
        return write_nifti(path, segmentation_array(labels, shape), affine)
