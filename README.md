...
the one in the dataset and the corresponding label values in `Subject_info_XXX.json` are
promoted from `2` ("automatically generated, without quality check") to `3` ("passed
quality check"). Rejected subjects leave the dataset untouched and are only recorded in the
audit trail.

Only the data schema is taken from BoneHub-Dataset, so none of its conversion or
segmentation stack is installed here.
...
- Validation of every upload (geometry match against the subject's image, size cap), with
  a backup of the previous segmentation before it is overwritten.
...
1. `POST /api/v1/subjects/next` — lease the next subject
2. `GET /api/v1/assignments/{id}/image` — download the image
3. `GET /api/v1/assignments/{id}/segmentation` — download the segmentation, if any
4. `POST /api/v1/assignments/{id}/submit` — send the verdict back
...
| `eligible_label_values` | `[2]` | Which `Subject_info` label values enter the queue |
| `confirmed_label_value` | `3` | Value written for labels a reviewer confirmed |
...
├── config.json           quality-check policy for this dataset folder
...
python -m unittest tests.test_submission.ConfirmedSubmissionTests.test_confirmed_labels_become_three_in_subject_info
```
...
| `test_queue.py` | Which subjects are queued, restricting the server to specific datasets, broken dataset folders |
| `test_assignment.py` | Who gets which subject, leases, expiry, release, requeue policy |
| `test_submission.py` | Confirmed submissions promoting labels to 3, rejections changing nothing, validation, partial uploads, audit trail |
